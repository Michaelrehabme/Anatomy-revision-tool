...
/** Inverse of the sRGB transfer curve, so an ID pixel maps back to its linear level. */
...
/**
 * Reads the ID pass back into one traced outline per strap.
 *
 * The renderer painted every strap in a flat colour: red carries the low
 * four bits of its index, green the high four, at sixteen linear levels.
 * Anti-aliased edge pixels fall between levels and decode to a neighbour;
 * they are single-pixel specks and the tracer's component floor drops them.
 */
function traceIds(dir: string): { name: string; polygons: number[][][]; area: number }[] {
  ...
  for (let i = 0; i < index.length; i++) {
    if (png.data[i * 4 + 3] < 128) continue; // background
    const r = Math.round(srgbToLinear(png.data[i * 4]) * 15);
    const g = Math.round(srgbToLinear(png.data[i * 4 + 1]) * 15);
    index[i] = g * 16 + r;
  }
  const straps: { name: string; polygons: number[][][]; area: number }[] = [];
  for (const [id, name] of Object.entries(legend)) {
    const k = Number(id);
    if (k === 1) continue; // the target has its own, cleaner mask
    const mask = new Uint8Array(index.length);
    let n = 0;
    for (let i = 0; i < index.length; i++) if (index[i] === k) { mask[i] = 1; n++; }
    if (n < 40) continue;
    const t = maskToPolygons(mask, png.width, png.height, { minComponentPx: 40, epsilon: 1.5, maxVertices: 60 });
    if (!t.polygons.length) continue;
    straps.push({ name: name.replace(/\.(l|r)$/, ''), polygons: t.polygons, area: t.area });
  }
  return straps;
}
...
for (const s of SUBJECTS) {
  ...
  for (const dir2 of readdirSync(dir, { withFileTypes: true })) {
    ...
    rotation.push({
      ...
      area: t.area,
      polygons: t.polygons,
      // Every other strap in this view, so a wrong click can be named.
      straps: traceIds(`${dir}/${dir2.name}`),
  ...
  out.push({
    ...s,
    straps: best ? best.straps : traceIds(dir),
  ...
  if (rotation.length) {
    const named = rotation.reduce((n, r) => n + (r.straps ? r.straps.length : 0), 0);
    console.log(`  ${named} neighbouring straps traced across ${rotation.length} angles`);
    console.log(
