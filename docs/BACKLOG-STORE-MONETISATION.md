...
**Settled 17 September: the 21 `clinical` sentences were removed.** They are `null` in `muscles.raw.json`, committed inside `a5d1856` — a commit whose message covers ligament plate framing and does not mention them, which is why this paragraph exists. They were the most exposed material in the file, not because the facts are obscure (tennis elbow at the common extensor origin is a definition) but because *which* pathology is paired with *which* muscle is a teaching selection, and the phrasing carried a voice: "gluteal amnesia in prolonged sitters", "eccentric strengthening is central to rehab". Removing them cost nothing measurable — `clinical.ts` builds its questions from `myotome` and `palpationNotes` and never reads `structure.clinical`, so the field was display-only, feeding a line on the muscle card and a sentence of MCQ explanation. All 686 tests pass and `validate-content` reports 0 errors and 0 warnings. The sentences are archived outside the repo should a properly referenced clinical layer be wanted later, at full coverage rather than 17%.

**`notes` was deliberately kept.** Of its 15 entries, six are spelling corrections *about* the source slides ("Spelled 'Trapezuis' on the source slide"), two are CR-013 verification records, four enumerate sub-muscles, and three are plain facts ("Largest muscle in the body"). That is the project owner's editorial metadata and unprotectable fact, not Maynard's prose — and the two CR-013 entries are written evidence of independent verification against a standard reference, which is the kind of paper this section wants more of, not less.

**Done 18 September: all 122 `actionText` sentences re-derived, and the dataset is now cut free.** `scripts/deriveActionText.mjs` builds each sentence from the muscle's own `actions` tags — a 64-term controlled vocabulary of anatomical facts — joined into natural phrasing, and never reads the old prose. Where a tag set is shared by several muscles (86 distinct sets across 122 muscles) it appends whichever of the muscle's own attachments actually distinguishes it, so the three vasti and the three finger extensors stay separable; `mcq.ts` draws its action-question distractors from these strings, and identical ones would collapse the question. Seven muscles whose tags hold opposing actions on one joint — the deltoid flexes AND extends the shoulder, true of the muscle and false of any of its fibres — are written out by hand from standard anatomy instead. Result: 122 distinct sentences where the deck-derived set had 113, no questions lost (8,675 before and after), 732 tests passing, content validation clean. Two are unchanged, "Opposes the thumb." and "Adducts the thumb.", which is the method landing on the only sensible phrasing and is the clearest sign those were never protectable.

**So permission is no longer the question.** Copyright protects expression, not fact. With the prose independently derived and the clinical commentary gone, there is nothing left to license, and writing to Salford or to Maynard to ask for rights would be requesting permission the project does not need — in writing, inviting a refusal that would then have to be honoured or visibly ignored. The earlier advice in this section to send that email is withdrawn. Keep the acknowledgement on the attributions page, which is accurate and generous, and treat any contact with Maynard as a professional courtesy rather than a licensing step.

**The remaining exposure is small and worth naming.** The `source` field on all 122 records still points at the deck and its slide numbers: that is honest provenance for where the project's *facts* were first gathered, not a claim about the current prose, and it is better kept than quietly deleted. Database right in the compilation belongs to whoever made the investment in it, and the compilation was the project owner's own. Neither is a reason to delay charging.
