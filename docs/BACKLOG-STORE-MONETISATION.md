...
**What remains is 122 `actionText` sentences, and the tags make it cheap.** Every muscle already carries an `actions` array — iliacus is `["hip-flexion", "hip-external-rotation"]` — so each sentence can be written from its own tag set plus a citable reference rather than paraphrased from the original, which is what makes the result independently derived rather than a reworded copy. Reducing `actionText` to the bare tags was considered and rejected: 122 muscles resolve to only 86 distinct tag-sets against 113 distinct sentences, so the collapse would make the three vasti, the four deep hip rotators and the five toe flexors indistinguishable from one another, and `mcq.ts` builds its action question with `actionText` as the correct answer and other muscles' `actionText` as distractors — a pool that degenerates once they collide. Rewriting from the scaffold keeps the discrimination and takes an afternoon rather than the one to two days estimated above.
